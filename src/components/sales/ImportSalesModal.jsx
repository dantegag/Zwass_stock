...
function autoDetect(headers) {
  const map = {}
  const HINTS = {
    product_name:   ['producto','product','nombre','name','descripcion','description','articulo','artículo'],
    ...
    quantity:       ['cantidad','qty','unidades','quantity','cant'],
    unit_price:     ['precio unit','precio unitario','p. unit','precio unit.','unit price','precio'],
    total_price:    ['total','importe','monto','total price'],
    currency:       ['moneda','currency','divisa'],
    payment_method: ['metodo','método','metodo de pago','método de pago','medio de pago','forma de pago','payment method'],
    nationality:    ['nacionalidad','nationality','origen','pais','país'],
...
function mapSaleRows(rows, columnMap) {
  return rows
    .map(row => {
      const p = {}
...
export default function ImportSalesModal({ onImport, onClose }) {
  ...
  const handleImport = async () => {
    setLoading(true)
    const mapped = mapSaleRows(rows, columnMap)
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="modal-enter bg-card border border-white/10 rounded-2xl w-full max-w-3xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="font-display text-2xl text-cream">Importar Ventas</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-muted text-xs mt-3 max-w-md">
                  Las ventas importadas se cargan con la fecha de hoy. Si el SKU coincide con un producto existente, el stock se descuenta automáticamente.
                </p>
              </div>
          ...
          {step === 'map' && (
            <div className="flex flex-col gap-5">
              <p className="text-muted text-sm">{rows.length} filas detectadas. Mapeá las columnas de tu archivo a los campos de la venta:</p>
            ...
            <div className="flex flex-col items-center gap-6 py-6">
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                <div className="bg-surface rounded-xl p-4 text-center border border-white/5">
                  ...
                  <p className="text-muted text-xs mt-1">Ventas importadas</p>
                </div>
                <div className="bg-surface rounded-xl p-4 text-center border border-white/5">
              ...
              {result.errorDetails?.length > 0 && (
                <div className="w-full max-w-md bg-surface/50 border border-danger/20 rounded-lg p-3">
                  <p className="text-danger text-xs uppercase tracking-wider mb-2">Detalle de errores</p>
          ...
          {step === 'map' && (
            <>
              ...
              <button
                ...
                disabled={loading || !columnMap.product_name || !columnMap.payment_method}
                ...
                title={(!columnMap.product_name || !columnMap.payment_method) ? 'Mapeá Producto y Método de Pago para continuar' : ''}
              >
